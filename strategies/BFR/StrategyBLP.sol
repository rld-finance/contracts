...
contract StrategyBLP is Manager, GasFeeThrottler, Stoppable {
    ...
    address public chef;
    address public blpRewardStorage;
    address public bfrRewardStorage;
    address public blpManager;
    address public bfrVault;
    ...
    constructor(
        address _token,        // 0x5402B5F40310bDED796c7D0F3FF6683f5C0cFfdf (staked blp)
        address _rewardToken,  // 0x82af49447d8a07e3bd95bd0d56f35241523fbab1 (weth)
        address _minter,       // 0xb95db5b167d75e6d04227cfffa61069348d271f5 (BFR reward router v2)
        address _chef,         // 0xA906F338CB21815cBc4Bc87ace9e68c87eF8d8F1 (BFR reward router)
        address _vault
    ) {
        token = _token;
        ...
        chef = _chef;
        vault = _vault;
        blpRewardStorage = IBFRRouter(chef).feeBlpTracker();
        bfrRewardStorage = IBFRRouter(chef).feeBfrTracker();
        blpManager = IBFRRouter(minter).blpManager();
        _giveAllowances();
    ...
    function _harvest() internal whenNotStopped {
        IBFRRouter(chef).compound();   // Claim and re-stake esBFR and multiplier points
        IBFRRouter(chef).claimFees();
    ...
    // mint more BLP with the ETH earned as fees
    function mintBlp() internal {
        uint256 rewardTokenBal = IERC20(rewardToken).balanceOf(address(this));
        if (rewardTokenBal > 0) {
            IBFRRouter(minter).mintAndStakeBlp(rewardToken, rewardTokenBal, 0, 0);
        }
    }
    ...
    // Always zero as you don't have to stake BLP
    ...
    function rewardsAvailable() public view returns (uint256) {
        uint256 rewardBLP = IBFRTracker(blpRewardStorage).claimable(address(this));
        uint256 rewardBFR = IBFRTracker(bfrRewardStorage).claimable(address(this));
        return rewardBLP + rewardBFR;
    }
    ...
    function _giveAllowances() internal {
        IERC20(rewardToken).safeApprove(blpManager, type(uint).max);
    }

    function _removeAllowances() internal {
        IERC20(rewardToken).safeApprove(blpManager, 0);
    }
}
